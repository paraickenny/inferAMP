...
def hello():
    ...
    print
    print "Enter names of reported amplified genes below. Genes should be in ALL CAPS and separated by commas."
    ...
    test_case = 0
    """
    answer = raw_input("Comma-separated list of genes (or number of test set):")
    ...
    """
    if test_case == 0:
        print "parsing comma separated list"
        #candidates = list(answer.split(","))
        query = request.args.get('query')
        print "received the following via http: ", query
        print type(query)
        query = query.encode('ascii','ignore')    #change imported text from unicode to ascii
        print "before map", query
    ...
    weboutput += str("<p> You entered the following amplified genes:" + str(candidates) +"<br>")
    nonamplifiedgenes = list(set(foundationlist) - set(
        candidates))  # assumes genes not flagged as amplified by Foundation are not amplified. Told this is true by Tech Support
    ...
    for a in regions:
        ...
        for g in a:
            ...
            if g[6] == "reported-up":
                ...
                weboutput += str("Reported amplified: " + g[4] + " (Chr" + g[0] + g[3] + ")<br>")
                #weboutput += "\n"
        ...
        weboutput += "%s" % ", ".join(map(str, cosmicinamp))
        #weboutput += "\n"
    #f = open("inferCNV_output.txt", "w")


    #for a in genome:
     #   line = str(a[0]) + "\t" + str(a[1]) + "\t" + str(a[2]) + "\t" + str(a[3]) + "\t" + str(a[4]) + "\t" + str(
     #       a[5]) + "\t" + str(a[6]) + "\t" + str(a[7]) + "\n"
    ...
    weboutput += "To run another analysis, click <a href=\"http://kennylab.org/infercnv-query.html\"> HERE</a> <br>"
